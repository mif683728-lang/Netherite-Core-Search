...
export async function performSearch(query: string, type: 'all' | 'images' | 'videos' | 'news' = 'all', page: number = 1): Promise<SearchResponse> {
  ...
  try {
    ...
    const actualType = type === 'news' ? 'all' : type;
    let prompt = `You are a search engine. Search for: ${query}. (Page ${page})
    ...
    if (actualType === 'images') {
      prompt = `Search for images related to: ${query}. (Page ${page})
      Return a JSON object with a "results" array. 
      Each object must have "title", "link" (the image URL), and "snippet".`;
    } else if (actualType === 'videos') {
      prompt = `Search for videos related to: ${query}. (Page ${page})
      Return a JSON object with a "results" array. 
